...
public class WallVisitorUtils {
	...
	public static void checkInsert(WallVisitor visitor, SQLInsertInto x) {
		...
		if (!visitor.getConfig().isInsertAllow()) {
			visitor.getViolations().add(
					new IllegalSQLObjectViolation(visitor.toSQL(x)));
		}
	}
	...
	private static void checkCondition(WallVisitor visitor, SQLExpr x) {
		...
		if (visitor.getConfig().isMustParameterized()) {
			ExportParameterVisitor exportParameterVisitor = visitor
					.getProvider().createExportParameterVisitor();
	...
	public static void checkConditionForMultiTenant(WallVisitor visitor,
		...
		if (parent instanceof SQLDeleteStatement) {
			...
			throw new IllegalStateException("not support parent : "
					+ parent.getClass());
		}
		...
		if (tableSource instanceof SQLExprTableSource) {
			...
			if (tableExpr instanceof SQLIdentifierExpr) {
				...
				if (ServletPathMatcher.getInstance().matches(
						tenantTablePattern, tableName)) {
		...
		if (tableSource.getAlias() != null) {
			left = new SQLPropertyExpr(new SQLIdentifierExpr(
					tableSource.getAlias()), visitor.getConfig()
					.getTenantColumn());
		} else {
			left = new SQLPropertyExpr(new SQLIdentifierExpr(matchTableName),
					visitor.getConfig().getTenantColumn());
		}
		...
		if (tenantValue instanceof Number) {
			...
			throw new IllegalStateException("tenant value not support type "
					+ tenantValue);
		}

		SQLBinaryOpExpr tenantCondition = new SQLBinaryOpExpr(left,
		...
		if (x == null) {
			...
			condition = new SQLBinaryOpExpr(tenantCondition,
					SQLBinaryOperator.BooleanAnd, x);
		}
	...
	public static void checkReadOnly(WallVisitor visitor,
		...
		if (tableSource instanceof SQLExprTableSource) {
			...
			SQLExpr tableNameExpr = ((SQLExprTableSource) tableSource)
					.getExpr();
	...
	public static Object getValue(SQLBinaryOpExpr x) {
		...
		if (x.getLeft() instanceof SQLName && x.getRight() instanceof SQLName) {
			if (x.getLeft().toString()
					.equalsIgnoreCase(x.getRight().toString())) {
		...
		if (x.getOperator() == SQLBinaryOperator.BooleanOr) {
			...
			for (;;) {
				if (left instanceof SQLBinaryOpExpr
						&& ((SQLBinaryOpExpr) left).getOperator() == x
								.getOperator()) {
		...
		if (x.getOperator() == SQLBinaryOperator.Equality) {
			if (x.getLeft() instanceof SQLNullExpr
					&& x.getRight() instanceof SQLNullExpr) {
				return true;
			}
	...
	public static Object getValue(SQLMethodInvokeExpr x) {
		...
		if ("len".equalsIgnoreCase(methodName)
				|| "length".equalsIgnoreCase(methodName)) {
	...
	public static boolean isSimpleCountTableSource(SQLSelect select) {
		...
		if (query instanceof SQLSelectQueryBlock) {
			...
			if (queryBlock.getSelectList().size() == 1) {
				SQLExpr selectItemExpr = queryBlock.getSelectList().get(0)
						.getExpr();
				if (selectItemExpr instanceof SQLAggregateExpr) {
					if (((SQLAggregateExpr) selectItemExpr).getMethodName()
	...
	public static void checkUnion(WallVisitor visitor, SQLUnionQuery x) {
		...
		if (WallVisitorUtils.queryBlockFromIsNull(x.getLeft())
				|| WallVisitorUtils.queryBlockFromIsNull(x.getRight())) {
			addViolation(visitor, x);
		}
	}
		...
		if (query instanceof SQLSelectQueryBlock) {
			...
			if (queryBlock.getSelectList().size() == 1
				...
				if (from instanceof SQLSubqueryTableSource) {
					SQLSelectQuery subQuery = ((SQLSubqueryTableSource) from)
							.getSelect().getQuery();
