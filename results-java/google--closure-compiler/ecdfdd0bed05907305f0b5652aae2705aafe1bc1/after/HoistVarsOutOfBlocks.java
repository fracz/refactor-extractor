...
class HoistVarsOutOfBlocks extends AbstractPostOrderCallback
  ...
  /**
   * Users of this class call {@link #process(Node, Node)} on it. That method executes a
   * {@link ReferenceCollectingCallback}. At the end of every scope, this method is executed and
   * causes another traversal of the scope, which does the hoisting.
   */
  @Override
  ...
  private void hoistVarIfNeeded(NodeTraversal t, Node varNode, Node parent) {
    ...
    for (Node lhs : varNode.children()) {
      ...
      if (rhs == null) {
        hoistRoot.addChildToFront(varNode.detach());
      } else {
