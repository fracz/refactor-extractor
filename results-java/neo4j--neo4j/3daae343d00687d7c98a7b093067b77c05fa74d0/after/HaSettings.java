...
public class HaSettings implements LoadableConfig
{
    ...
    public static final Setting<Duration> state_switch_timeout = setting( "ha.role_switch_timeout", DURATION, "120s" ).build();
    ...
    public static final Setting<Duration> internal_state_switch_timeout =
            setting( "ha.internal_role_switch_timeout", DURATION, "10s" ).build();

    @Description( "Timeout for taking remote (write) locks on slaves. Defaults to ha.slave_read_timeout." )
    public static final Setting<Duration> lock_read_timeout = setting( "ha.slave_lock_timeout", DURATION ).inherits( read_timeout ).build();
    ...
    public static final Setting<Integer> max_concurrent_channels_per_slave =
            setting( "ha.max_channels_per_slave", INTEGER, "20" ).constraint( min( 1 ) ).build();

    @Description( "Hostname and port to bind the HA server." )
    public static final Setting<HostnamePort> ha_server = setting( "ha.host.data", HOSTNAME_PORT, "0.0.0.0:6001-6011" ).build();
    ...
    public static final Setting<Boolean> slave_only = setting( "ha.slave_only", BOOLEAN, Settings.FALSE ).build();
    ...
    public static final Setting<BranchedDataPolicy> branched_data_policy = setting( "ha.branched_data_policy",
            options( BranchedDataPolicy.class ), "keep_all" ).build();
    ...
    public static final Setting<Boolean> ha_status_auth_enabled =
            setting( "dbms.security.ha_status_auth_enabled", BOOLEAN, Settings.TRUE ).build();
    ...
    public static final Setting<Long> com_chunk_size = setting( "ha.data_chunk_size", BYTES, "2M" ).constraint( min( 1024L ) ).build();

    @Description( "Interval of pulling updates from master." )
    public static final Setting<Duration> pull_interval = setting( "ha.pull_interval", DURATION, "0s" ).build();

    @Description( "The amount of slaves the master will ask to replicate a committed transaction. " )
    public static final Setting<Integer> tx_push_factor = setting( "ha.tx_push_factor", INTEGER, "1" ).constraint( min( 0 ) ).build();
    ...
    public static final Setting<TxPushStrategy> tx_push_strategy =
            setting( "ha.tx_push_strategy", options( TxPushStrategy.class ), fixed_ascending.name() ).build();
    ...
    public static final Setting<BranchedDataCopyingStrategy> branched_data_copying_strategy =
            setting( "ha.branched_data_copying_strategy", options( BranchedDataCopyingStrategy.class ), branch_then_copy.name() ).build();

    @Description( "Size of batches of transactions applied on slaves when pulling from master" )
    public static final Setting<Integer> pull_apply_batch_size = setting( "ha.pull_batch_size", INTEGER, "100" ).build();
