...
package com.weibo.api.motan.registry.zookeeper.command;
...
@SpiMeta(name = "commandZookeeper")
public class CommandZookeeperRegistryFactory extends AbstractRegistryFactory {
    ...
    protected Registry createRegistry(URL registryUrl) {
        try {
            ...
            ZkClient client = new ZkClient(registryUrl.getParameter("address"), sessionTimeout, timeout);
            return new CommandZookeeperRegistry(registryUrl, client);
        } catch (ZkException e) {
            LoggerUtil.error("[CommandZookeeperRegistry] fail to connect zookeeper, cause: " + e.getMessage());
            throw e;
        }
    }
}