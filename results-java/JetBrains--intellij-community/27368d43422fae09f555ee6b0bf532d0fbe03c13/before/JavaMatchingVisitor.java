...
public class JavaMatchingVisitor extends JavaElementVisitor {
  ...
  public void visitNameValuePair(PsiNameValuePair pair) {
    ...
    if (nameIdentifier != null) {
      ...
      if (handler instanceof SubstitutionHandler) {
        myMatchingVisitor
          .setResult(((SubstitutionHandler)handler).handle(((PsiNameValuePair)myMatchingVisitor.getElement()).getNameIdentifier(),
                                                           myMatchingVisitor.getMatchContext()));
      }
      else {
        myMatchingVisitor
          .setResult(myMatchingVisitor.match(nameIdentifier, ((PsiNameValuePair)myMatchingVisitor.getElement()).getNameIdentifier()));
      }
    }
    else {
      myMatchingVisitor.setResult(true);
      ...
      myMatchingVisitor.getMatchContext().setUnmatchedElementsListener(
        mylistener = new MatchContext.UnmatchedElementsListener() {
          ...
          public void matchedElements(List<PsiElement> elementList) {
            if (elementList != null) {
              if (myUnmatchedList == null) {
                myUnmatchedList = new LinkedList<PsiElement>(elementList);
              }
              else {
                myUnmatchedList.addAll(elementList);
              }
            }
          }
