...
public class TooBroadCatchInspection extends StatementInspection {
  ...
  protected String buildErrorString(PsiElement location) {
    ...
    final PsiParameter[] parameters = tryStatement
    ...
    if (typesMasked.size() == 1) {
      return InspectionGadgetsBundle.message("too.broad.catch.problem.descriptor", typesMasked.get(0));
    }
    else {
      ...
      return InspectionGadgetsBundle
        .message("too.broad.catch.problem.descriptor1", typesMaskedString, typesMasked.get(typesMasked.size() - 1));
    }
  }
  ...
  private static class TooBroadCatchVisitor
    ...
    public void visitTryStatement(@NotNull PsiTryStatement statement) {
      ...
      final Set<PsiType> exceptionsCaught = new HashSet<PsiType>(
        numExceptionsThrown);
      final PsiParameter[] parameters = statement
        .getCatchBlockParameters();
      for (final PsiParameter parameter : parameters) {
        final PsiType typeCaught = parameter.getType();
        if (exceptionsThrown.contains(typeCaught)) {
        ...
        for (PsiType typeThrown : exceptionsThrown) {
          if (exceptionsCaught.contains(typeThrown)) {
            ...
            exceptionsCaught.add(typeCaught);
            final PsiTypeElement typeElement = parameter
              .getTypeElement();
