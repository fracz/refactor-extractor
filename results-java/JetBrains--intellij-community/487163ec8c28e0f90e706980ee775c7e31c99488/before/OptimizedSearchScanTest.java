...
public class OptimizedSearchScanTest extends StructuralSearchTestCase {
  ...
  public void testOptionalMethodWithThrowsClause() {
    final String plan = findWordsToBeUsedWhenSearchingFor(
      ...
      "}");
    assertEquals("exception should not be in plan", "[in code:interface|in code:enum|in code:class][in code:C]", plan);
    ...
    assertEquals("throws should not be in plan",
                 "[in code:interface|in code:enum|in code:class][in code:C][in code:m][in code:String][in code:println][in code:out]" +
  ...
  public void testExtendsImplements() {
    ...
    assertEquals("extends should not be in plan", "[in code:interface|in code:enum|in code:class][in code:A]", plan1);

    final String plan2 = findWordsToBeUsedWhenSearchingFor("class B implements '_I{0,0} {}");
    assertEquals("implements should not be in plan", "[in code:interface|in code:enum|in code:class][in code:B]", plan2);
  }
  ...
  public void testClasses() {
    ...
    assertEquals("[in code:interface|in code:enum|in code:class][in code:A]", plan1);
