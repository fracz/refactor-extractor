 ...
 */
public class ODatabaseDocumentEmbedded extends OListenerManger<ODatabaseListener> implements ODatabaseDocumentInternal {
  ...
  public ODatabaseDocument setConflictStrategy(final String iStrategyName) {
    checkIfActive();
    getStorage().setConflictStrategy(Orient.instance().getRecordConflictStrategy().getStrategy(iStrategyName));
    return this;
  }

  public ODatabaseDocument setConflictStrategy(final ORecordConflictStrategy iResolver) {
    checkIfActive();
  ...
  public <RET extends ORecord> RET executeSaveEmptyRecord(ORecord record, String clusterName) {
    ...
    final OStorageOperationResult<OPhysicalPosition> ppos = storage
        .createRecord(rid, content, record.getVersion(), ODocument.RECORD_TYPE, OPERATION_MODE.SYNCHRONOUS.ordinal(), null);
  ...
  public ODatabaseDocument begin() {
    return begin(OTransaction.TXTYPE.OPTIMISTIC);
  }

  public ODatabaseDocument begin(final OTransaction.TXTYPE iType) {
    checkOpeness();
    checkIfActive();
   ...
   * @see #setMVCC(boolean), {@link #isMVCC()}
   */
  public ODatabaseDocument delete(final ORecord record) {
    checkOpeness();
    if (record == null)
  ...
  public OUncompletedCommit<Void> initiateCommit(boolean force) {
    ...
    if (!force && currentTx.amountOfNestedTxs() > 1)
      return new ODatabaseDocumentEmbedded.UncompletedCommit(false, currentTx.initiateCommit(false));
    ...
    try {
      return new ODatabaseDocumentEmbedded.UncompletedCommit(true, currentTx.initiateCommit(force));
    } catch (RuntimeException e) {
      OLogManager.instance().debug(this, "Error on transaction commit", e);
   ...
   */
  @Override
  public ODatabaseDocument activateOnCurrentThread() {
  ...
  private class UncompletedCommit implements OUncompletedCommit<Void> {
    ...
    public Void complete() {
      ...
      try {
        ...
        for (ODatabaseListener listener : browseListeners())
          try {
            listener.onBeforeTxRollback(ODatabaseDocumentEmbedded.this);
          } catch (Throwable t) {
            OLogManager.instance().error(this, "Error before transaction rollback", t);
          }
        ...
        for (ODatabaseListener listener : browseListeners())
          try {
            listener.onAfterTxRollback(ODatabaseDocumentEmbedded.this);
          } catch (Throwable t) {
            OLogManager.instance().error(this, "Error after transaction rollback", t);
          }
        throw e;
      ...
      for (ODatabaseListener listener : browseListeners())
        try {
          listener.onAfterTxCommit(ODatabaseDocumentEmbedded.this);
        } catch (Exception e) {
          final String message =
              "Error after the transaction has been committed. The transaction remains valid. The exception caught was on execution of "
                  + listener.getClass() + ".onAfterTxCommit()";
