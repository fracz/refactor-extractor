...
public class ShiftTransition implements Transition {
  ...
  public State apply(State state, double scoreDelta) {
    return new State(state.stack.push(state.sentence.get(state.tokenPosition)), state.transitions.push(this), state.sentence, state.tokenPosition + 1, state.score + scoreDelta, false);
  }
