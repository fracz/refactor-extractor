...
public class XRenderer extends AxisRenderer {
    ...
    protected float defineStaticLabelsPosition(float axisCoordinate, int distanceToAxis){
        ...
        if(labelsPositioning == LabelPosition.INSIDE) { // Labels sit inside of chart
            ...
            if(style.hasXAxis())
                result -=  style.getAxisThickness() /2;//TODO este codigo esta repetido no if debaixo, da para melhorar isto
    ...
    protected float measureInnerChartRight(int right){
        ...
        if(labelsPositioning != LabelPosition.NONE && borderSpacing + mandatoryBorderSpacing < lastLabelWidth / 2)
            rightBorder = lastLabelWidth/2 - (borderSpacing + mandatoryBorderSpacing); //TODO dá para melhorar codigo, subtract lastLabelWidht directly
