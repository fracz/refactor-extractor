 ...
 */
 public static function initSettings()
 {
  Daemon::$settings = array(
   ...
   'defaultpidfile' => '/var/run/phpdaemon.pid',
 ...
 */
 ...
 {
  if ($s === '') {return '';}
  if (Daemon::$settings['obfilterauto'] && (Daemon::$req !== NULL)) {Daemon::$req->out($s,FALSE);}
  else {Daemon::log('Unexcepted output (len. '.strlen($s).'): \''.$s.'\'');}
  return '';
 }
 /* @method header
    @param string
 ...
 */
 ...
 {
  if (Daemon::$req === NULL) {throw new Exception('Daemon::header() called out of request context');}
 ...
 */
 public static function compatRunEmul()
 {
  ...
  if (isset($args[$k = 'configfile'])) {Daemon::$settings[$k] = $args[$k];}
  if (isset(Daemon::$settings['configfile']) && !Daemon::loadConfig(Daemon::$settings['configfile']))
  {
   $error = TRUE;
  }
  if (!Daemon::loadSettings($args)) {$error = TRUE;}
  if (!isset(Daemon::$settings['path'])) {exit('\'path\' is not defined');}
 ...
 */
 public static function loadConfig($path)
 {
  ...
  if (!Daemon::loadSettings($cfg)) {return FALSE;}
  return TRUE;
 }
 /* @method loadSetings
 ...
 */
 public static function loadSettings($settings)
 {
  ...
  foreach ($settings as $k => $v)
  {
   ...
   {
    if ($k === 'maxmemoryusage') {Daemon::$parsedSettings[$k] = Daemon::parseSize($v);}
    elseif ($k === 'maxrequests') {Daemon::$parsedSettings[$k] = Daemon::parseNum($v);}
    elseif ($k === 'autogc') {Daemon::$parsedSettings[$k] = Daemon::parseNum($v);}
    elseif ($k === 'maxidle') {Daemon::$parsedSettings[$k] = Daemon::parseTime($v);}
    elseif ($k === 'autoreload') {Daemon::$parsedSettings[$k] = Daemon::parseTime($v);}
    elseif ($k === 'maxconcurrentrequestsperworker') {Daemon::$parsedSettings[$k] = Daemon::parseNum($v);}
    elseif ($k === 'keepalive') {Daemon::$parsedSettings[$k] = Daemon::parseTime($v);}
    elseif ($k === 'mpmdelay') {Daemon::$parsedSettings[$k] = Daemon::parseTime($v);}
    elseif ($k === 'chunksize') {Daemon::$parsedSettings[$k] = Daemon::parseSize($v);}
    if (is_int(Daemon::$settings[$k])) {Daemon::$settings[$k] = (int) $v;}
    else {Daemon::$settings[$k] = $v;}
   }
   elseif (strpos($k,'mod') === 0) {Daemon::$settings[$k] = $v;}
   else
   {
  ...
  if (isset($settings['path']))
  {
   ...
   else {Daemon::$pathReal = realpath(Daemon::$settings['path']);}
  }
  return !$error;
 }
 /* @method openLogs
 ...
 */
 ...
 {
  $l = substr($str,-1);
  if (($l === 'k') || ($l === 'K')) {return ((int) substr($str,0,-1)*100);}
  if (($l === 'm') || ($l === 'M')) {return ((int) substr($str,0,-1)*1000*1000);}
  if (($l === 'G') || ($l === 'G')) {return ((int) substr($str,0,-1)*1000*1000*1000);}
  return (int) $str;
 }
 /* @method parseTime
   ...
   for ($i = 0; $i < $bufsize; ++$i)
   {
    ...
    elseif ($code === 2) // busy
    {
     ...
     ++$stat['busy'];
		...
		elseif ($code === 3) // shutdown
		{
		 ++$stat['shutdown'];
    }
    elseif ($code === 4) // pre-init
		{
		 ++$stat['alive'];
		 ++$stat['preinit'];
		 ++$stat['idle'];
    }
    elseif ($code === 5) // wait-init
		{
		 ++$stat['alive'];
		 ++$stat['waitinit'];
		 ++$stat['idle'];
