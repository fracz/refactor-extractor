...
class AsyncStream
{
 ...
 public function initStream($readFD = NULL,$writeFD = NULL)
 {
  if (is_string($readFD))
  {
   ...
   if ($u['scheme'] === 'unix')
   {
    if (!Daemon::$useSockets) {$readFD = stream_socket_client($readFD,$errno,$errstr,1);}
    else
    {
   ...
   elseif (($u['scheme'] === 'tcp') || (($u['scheme'] === 'tcpstream')))
   {
    if (!Daemon::$useSockets || ($u['scheme'] === 'tcpstream')) {$readFD = stream_socket_client('tcp://'.substr($readFD,12),$errno,$errstr,1);}
    else
    {
 ...
 {
  if (!$this->writeState) {$this->closeWrite();}
 ...
 public function setFD($readFD,$writeFD = NULL)
 {
  ...
  {
   if (!stream_set_blocking($this->readFD,0)) {throw new Exception('setting blocking for read stream failed');}
   if (!event_buffer_fd_set($this->readBuf,$this->readFD)) {throw new Exception('setting descriptor for write buffer failed');}
  }
  if ($this->writeFD === NULL) {return $this;}
 ...
 public function setPriority($read,$write)
 {
  ...
  if ($this->readBuf !== NULL)
  {
   if (event_buffer_priority_set($this->readBuf,$this->readPriority) === FALSE) {throw new Exception('setting priority for read buffer failed');}
  }
  if ($this->writeBuf !== NULL)
  {
   if (event_buffer_priority_set($this->writeBuf,$this->writePriority) === FALSE) {throw new Exception('setting priority for read buffer failed');}
  }
  return $this;
 }
 ...
 {
  if ($low === NULL) {$low = 1;}
  if ($high === NULL) {$high = 0xFFFFFF;}
  if (!event_buffer_watermark_set($this->readBuf,EV_READ,$low,$high)) {throw new Exception('readMask('.$low.','.$high.') failed.');}
  return $this;
 }
 public function writeMask($low = NULL,$high = NULL)
 {
  if ($low === NULL) {$low = 1;}
  if ($high === NULL) {$high = 0xFFFFFF;}
  if (!event_buffer_watermark_set($this->writeBuf,EV_WRITE,$low,$high)) {throw new Exception('writeMask('.$low.','.$high.') failed.');}
  return $this;
 }
 public function read($n = NULL)
 {
  ...
  if ($r === NULL) {$r = '';}
  if ($r === FALSE) {throw new Exception('read buffer failed.');}
  return $r;
 }
 public function write($s)
 {
  ...
  if ($s !== '') {$this->writeState = TRUE;}
  if (!event_buffer_write($b,$s)) {throw new Exception('write() failed.');}
  return $this;
 }
 public function onEofEvent()
 {
  $this->EOF = TRUE;
  if ($this->onEOF !== NULL) {call_user_func($this->onEOF,$this);}
  $this->closeRead();
 }
 public function eof()
 {
  if (!$this->EOF && (($this->readFD === FALSE) || feof($this->readFD)))
  {
   $this->onEofEvent();
  }
  return $this->EOF;
 ...
 public function onWriteEvent($buf,$arg = NULL)
 {
  ...
  if ($this->finishWrite) {$this->closeWrite();}
 }
