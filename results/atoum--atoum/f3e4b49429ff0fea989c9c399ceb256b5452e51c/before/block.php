...
abstract class block extends data implements \iteratorAggregate
{
	...
	{
		return new iterator($this->getByTag($tag));
	}
	...
	public function getIterator() {
		return new iterator(array_values($this->children));
	}
	...
	public function isChild(data $child)
	{
		return ($child->parent === $this);
	}

	public function checkChild(data $child)
	{
	...
	public function addChild(data $child)
	{
		if ($this->checkChild($child) === false)
		{
	...
	public function deleteChild(data $child)
	{
	...
	protected function tagExists($tag, & $tags)
	{
		...
		if (sizeof($tags) <= 0)
		{
			throw new exceptions\logic('There is no tag \'' . $tag . '\' defined');
		}
	}
}
