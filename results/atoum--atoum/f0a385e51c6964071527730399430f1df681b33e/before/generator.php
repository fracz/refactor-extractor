		...
		$this
			...
			->then
				->when(function() use ($dummy) { $dummy->bar(); })
					->mock($calls)->call('addCall')->withArguments(new atoum\test\adapter\call('bar', array()))->once()
				->when(function() use ($dummy) { $dummy->bar(); })
					->mock($calls)->call('addCall')->withArguments(new atoum\test\adapter\call('bar', array()))->twice()
		;
	}
