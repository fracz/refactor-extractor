<div id="comment_{{ $comment->getId() }}" class="well clearfix" data-pk="{{ $comment->getId() }}">
    @if (Sentry::check() && Sentry::getUser()->hasAccess('mod'))
        <div class="col-md-10 col-sm-9">
            ...
            <p id="editable_comment_{{ $comment->getId() }}" class="editable" data-pk="{{ $comment->getId() }}" data-url="{{ URL::route('blog.posts.comments.update', array('posts' => $post_id, 'comments' => $comment->getId())) }}">{{ nl2br(e($comment->getBody())) }}</p>
        </div>
        <div class="hidden-xs">
