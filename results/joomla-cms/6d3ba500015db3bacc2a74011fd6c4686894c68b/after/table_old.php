...
$mParams->def('display_num', 		$limit);
	...
	?>
	<table width="100%" cellpadding="4" cellspacing="0" border="1" align="center" class="contentpane<?php echo $pageclass_sfx; ?>">
	<?php
	if ($hasHeadBlock) {
		?>
		<tr>
			<td width="100%" valign="top" class="contentdescription<?php echo $pageclass_sfx; ?>">
				<?php
				// show image
	...
	?>
		...
		<form action="index.php?option=com_contact&amp;catid=<?php echo $categoryId;?>&amp;Itemid=<?php echo $Itemid;?>" method="post" name="adminForm">
		...
		for ($i = 0; $i < $nContacts; $i++) {
			...
			<tr>
				...
				if ( $mParams->get( 'email' ) ) {
					...
					?>
					<td width="20%" class="sectiontableentry<?php echo $k+1; ?>">
				...
				if ( $mParams->get( 'telephone' ) ) {
					?>
					<td width="15%" class="sectiontableentry<?php echo $k+1; ?>">
		...
		<tr>
			<td>
				...
				if ( ( $mParams->get( 'type' ) == 'category' ) && $mParams->get( 'other_cat' ) ) {
					$this->showCategories( $mParams, $categories, $categoryId );
