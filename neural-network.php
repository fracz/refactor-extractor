...
$n = new NeuralNetwork(32, 40, 12, 1);
$n->setVerbose(false);

$input = file_get_contents(__DIR__ . '/results/delta.csv');
$inputData = explode(PHP_EOL, $input);
array_shift($inputData);
shuffle($inputData);

foreach ($inputData as $line) {
    $delta = array_map('floatval', explode(',', $line));
    $output = 1;
	if (rand(0, 100) % 2) {
        $delta = array_map(function($a) { return -$a; }, $delta);
        $output = 0;
	}
    $n->addTestData($delta, [$output]);
}
...
echo "Learning the Code Quality function".PHP_EOL;
// train the network in max 1000 epochs, with a max squared error of 0.01
while (!($success = $n->train(1000, 0.31)) && ++$i<$max) {
...
for ($i = 0; $i < min(10, count($n->trainInputs)); $i ++) {
