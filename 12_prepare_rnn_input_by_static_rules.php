...
$readDataset = function ($fileBefore, $fileAfter) use ($maxLength, $astDir, $tokensTreatedAsBad) {
    ...
    for ($i = 0; $i < $count; $i++) {
        $tokenCount = max(substr_count($rowsBefore[$i], ','), substr_count($rowsAfter[$i], ',')) + 1;
        if ($tokenCount < 3 || $tokenCount > $maxLength) {
            unset($rowsBefore[$i]);
            unset($rowsAfter[$i]);
        }
    }
    ...
    for ($i = 0; $i < count($rowsBefore); $i++) {
        $costBefore = calculateCost(explode(',', $rowsBefore[$i]));
        $costAfter = calculateCost(explode(',', $rowsAfter[$i]));
        if ($costBefore != $costAfter) {
            $beforeIsBetter = $costBefore < $costAfter;
            $result[] = [
                'X' => implode(',', array_pad(explode(',', $rowsBefore[$i]), $maxLength, 0)),
                'Y' => implode(',', $beforeIsBetter ? [1, 0] : [0, 1]),
                'len' => substr_count($rowsBefore[$i], ',') + 1,
            ];
            $result[] = [
                'X' => implode(',', array_pad(explode(',', $rowsAfter[$i]), $maxLength, 0)),
                'Y' => implode(',', $beforeIsBetter ? [0, 1] : [1, 0]),
